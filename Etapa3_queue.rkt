...
(define (dequeue q)
  (cond [(null? (queue-left q)) (define qq (struct-copy queue q
                          [left (cdr (foldl cons (queue-left q) (queue-right q)))]
                          [right null]
                          [size-r 0]
                          [size-l (- (queue-size-r q) 1)])) qq]
        [else (define qq (struct-copy queue q
                         [left (cdr (queue-left q))]
                         [size-l (- (queue-size-l q) 1)]))
 qq]))

;(foldl cons (queue-left q) (queue-right q))
...
(define (top q)
  (cond [(null? (queue-left q)) 0]
        [else (first (queue-left q))]))
