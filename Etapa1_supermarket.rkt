...
(define (min-helper counters)
  (sort counters
        (lambda(c1 c2)
          (< (counter-tt c1) (counter-tt c2)))))

(define (min-tt counters)
  (define CC (first(min-helper counters)))(cons(counter-index CC) (counter-tt CC)))
...
(define (add-helper C name n-items)
  (match C
    [(counter index tt queue)
     (define Cc (struct-copy counter C[queue (append queue (list(cons name n-items)))][tt (+ tt n-items)])) Cc]))


(define (add-to-counter C name n-items)
  (add-helper C name n-items)
  )
...
(define (serve requests C1 C2 C3 C4)
  ...
  (define CC (first(min-helper counters)))(counter-tt CC))
  ...
  ; - avantaj: puteți să vă testați fiecare funcție imediat ce ați implementat-  

  (if (null? requests)
      (list C1 C2 C3 C4)
      (match (car requests)
        [(list 'delay index minutes) (cond
                                       ((= (counter-index C1) index) (serve (cdr requests) (tt+ C1 minutes) C2 C3 C4))
                                       ((= (counter-index C2) index) (serve (cdr requests) C1 (tt+ C2 minutes) C3 C4))
                                       ((= (counter-index C3) index) (serve (cdr requests) C1 C2 (tt+ C3 minutes) C4))
                                       (else (serve (cdr requests) C1 C2 C3 (tt+ C4 minutes))))
                                     ]
        [(list name n-items) (cond
                               [(and (= (counter-tt C1) (min-counter-tt(list C1 C2 C3 C4)))(<= n-items ITEMS)) (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4)]
                               [(= (counter-tt C2) (min-counter-tt(list C1 C2 C3 C4))) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4)]
                               [(= (counter-tt C3) (min-counter-tt(list C1 C2 C3 C4))) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4)]
                               [else(= (counter-tt C4) (min-counter-tt(list C1 C2 C3 C4))) (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items))]
                               )])))
