...
(define (rotate left right Acc)
  (cond [(stream-empty? left) (reverse right)]
        [else (stream-append left (reverse right) Acc)]
        ))
...
(define (enqueue x q)
  (define qq (struct-copy queue q
                          [right (append (list x) (queue-right q))]
                          [size-r (+ (queue-size-r q) 1)]))
  (cond [( < (queue-size-l qq) (queue-size-r qq))
         (make-queue (rotate (queue-left qq) (queue-right qq) null) null (+ (queue-size-l qq) (queue-size-r qq)) 0)]
        [else qq]))
...
(define (dequeue q)
  (cond [(null? (queue-left q)) (define qq (struct-copy queue q
                                                        [left (stream-rest (foldl cons (queue-left q) (queue-right q)))]
                                                        [right null]
                                                        [size-r 0]
                                ...
                                (cond [( < (queue-size-l qq) (queue-size-r qq))
                                       (make-queue (rotate (queue-left qq) (queue-right qq) null) null (+ (queue-size-l qq) (queue-size-r qq)) 0)]
                                      [else qq])]
        [else (define qq (struct-copy queue q
...
(define (top q)
  (cond [(stream-empty? (queue-left q)) 0 ]
        [else (stream-first (queue-left q))]))
