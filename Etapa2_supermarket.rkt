...
(define min-tt (lambda (counters) (general-func min-helper-tt counters))) ; folosind funcția de mai sus
(define min-et (lambda (counters) (general-func min-helper-et counters))) ; folosind funcția de mai sus
...
(define (remove-first-from-counter C)
  (cond ((null? (cdr(counter-queue C))) (empty-counter (counter-index C)))
        (else (make-counter (counter-index C)
                            (mySum(cdr(map cdr (counter-queue C))))
                            (cdadr(counter-queue C)) (cdr (counter-queue C))))))
